...
export default function LoginPage() {
    ...
    const handleLogin = async (e) => { //function triggered when login form is submitted
        ...
        try { 
            const response = await fetch('http://localhost:4000/api/v1/users/login', { 
                ...
                body: JSON.stringify({ //sends login data as JSON string
                    usernameOrEmail,
                    password,
                }),
            });
            ...
            if (response.ok) {
                router.push('/'); //upon successful login, redirects
            } else {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-r from-[#001b29] to-[#000E14] p-4">
        <form onSubmit={handleLogin} className='space-y-4 p-8 rounded-lg shadow-lg w-full max-w-md'>
        ...
        <div className="flex justify-between bg-gray-100 rounded-full overflow-hidden">
            <button type='button' className="w-1/2 bg-gradient-to-r from-[#00628d] to-[#003047] text-white py-2">Login</button>
            <button
        ...
        <button type='submit' className="w-full py-2 bg-gradient-to-r from-[#00628d] to-[#003047] text-white rounded-lg cursor-pointer">
            Login
        </button>
